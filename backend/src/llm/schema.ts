  ...
  properties: {
    ...
    companies: {
      ...
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['name', 'legal_form', 'bin', 'role', 'sentiment', 'quote', 'confidence'],
        properties: {
          name: { type: 'string' },
...
export const eventExtractSchema = z.object({
  ...
  amount_kzt: z.number().nonnegative().nullish(),
  quote: z.string().min(1).max(4000),
  confidence: confidenceSchema,
});
