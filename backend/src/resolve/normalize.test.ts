...
describe('isJunkName', () => {
  ...
  it('отсекает органы власти верхнего уровня с уточнением', () => {
    // Реальный случай: «правительство РФ» получало роль заказчика. Точное
    // совпадение со стоп-словом «правительство» его не ловило.
    for (const name of ['правительство РФ', 'Правительство Москвы', 'Минстрой России', 'Госдума']) {
      expect(isJunkName(normalizeName(name)), name).toBe(true);
    }
  });

  it('не отсекает конкретный департамент — он бывает настоящим заказчиком', () => {
    // По госконтракту заказчиком выступает конкретное ведомство. Отсечь его
    // значило бы потерять самый ценный факт в карточке объекта.
    for (const name of [
      'Департамент строительства Москвы',
      'Департамент градостроительной политики',
      'Управление капитального строительства',
    ]) {
      expect(isJunkName(normalizeName(name)), name).toBe(false);
    }
  });

  it('«ГК» срезается как организационная форма', () => {
    // Устаревшая нормализация без «ГК» разнесла «ГК Консоль» и «Консоль»
    // на две карточки с разными ключами.
    expect(normalizeName('ГК Консоль').key).toBe(normalizeName('Консоль').key);
  });
});
