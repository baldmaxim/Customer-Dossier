...
import { ASSERTION_STATUSES, REVIEW_DECISIONS } from '../assertions/model.js';
import {
...
const listSchema = z.object({
  ...
  predicate: z.enum(['participates_in_project', 'event', 'company_mentioned', 'project_mentioned']).optional(),
  ...
  const items = await query(
    `SELECT ${ASSERTION_COLUMNS} ${ASSERTION_FROM}
     WHERE ($1::assertion_status IS NULL OR a.status = $1::assertion_status)
       ...
       AND ($5::bigint IS NULL OR $5::bigint IN (a.subject_project_id, a.object_project_id))
