...
ADR-004 (запуски извлечения и публикация наборов), ADR-005 (идентичность и безопасное слияние), ADR-006 (адаптеры сайтов), ADR-007 (Telegram: курсоры, журнал бота), ADR-008 (смысл связей, время, события), ADR-009 (объяснимые сигналы), ADR-010 (обращения и рабочее досье), ADR-011 (схема связей, снимки, выгрузки), ADR-012 (источник-реестр).
...
### Этап 20A — источник-реестр наш.дом.рф (2026-09-21)

По задаче владельца открыт новый путь сбора: реестр ЕИСЖС как источник (`mode: registry_api`, `ingest/registry/*`,
миграция `025_registry_source.sql`, ADR-012). Реализовано и проверено офлайн: typecheck 0, `npm test` 623/623,
включая 16 новых проверок. Канон этим этапом не пишется, источник не заведён и не допущен.
Отчёт — `stages/20A_REPORT.md`, прогон пользователя — `TESTING_LOCAL.md` раздел **O**.
Контрольная точка: живая проба. Из среды агента наш.дом.рф ответил 403; ответ с машины пользователя решает,
начинается ли этап 20B (детерминированная запись в канон и карточка объекта).

### Этап 20B — карточки из реестра (2026-09-21)

Данные реестра доходят до экрана: миграция `026_registry_canon.sql` (происхождение `registry`, роль
`developer`), `backend/src/registry/*` (детерминированная запись канона, разница снимков, чтение),
блок «Данные реестра» на карточках объекта и компании. Модель не участвует, рубильник —
`REGISTRY_PUBLISH_ENABLED`. Отчёт — `stages/20B_REPORT.md`, прогон пользователя — `TESTING_LOCAL.md`
раздел **P**.
