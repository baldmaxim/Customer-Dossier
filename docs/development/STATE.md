...
## Этап 22 — поток и карточка (22.09.2026)

Работа по разбору слабых мест: почему разбор шёл «по запросу», почему в интерфейсе мусор,
что должно быть в карточке компании.

| Что | Итог |
|---|---|
| Разбор при выключенной модели | Проход пропускается целиком (`probeModel`, `checkLlmConnection` 3 с). Раньше запуски ставились, падали и уносили редакцию из автопотока навсегда: автопостановка берёт только редакции без единого запуска |
| Упавший запуск | Возвращается в поток сам: `retryFailedRuns` → `retryRun`, пауза `REPROCESS_RETRY_BACKOFF_MIN=15`, потолок `REPROCESS_RETRY_MAX=3` неудач на редакцию, `cancelled` не повторяется. Рубильник `REPROCESS_RETRY_ENABLED` |
| Ступень «Обработка» на `/admin` | Считает состояния последних редакций (нет ИИ-допуска, ждёт, упало с повтором, попытки исчерпаны, в карточках, не о стройке) + причины падений за неделю + отвечает ли модель. Прежний счётчик читал `raw_documents.status` — колонку старого конвейера, которую новый путь не меняет |
| Лента компании | `GET /api/companies/:id/publications` поверх публикаций и опубликованных утверждений. Прежний блок «Упоминания» читает `mentions`, куда пишет только заблокированный legacy `apply.ts`, — на данных нового конвейера он пуст всегда |
| Контрагенты | `GET /api/companies/:id/partners`: договор, корпоративная связь и совместное участие раздельно, с подписью «вместе на объекте — не договор» |
| Карточка компании | Две вкладки: «Обзор» (сводка, лента, контрагенты) и «Подробно» (сигналы, досье, объекты, события, старые упоминания, схема). Числа не менялись |
| Экран «Подрядчики» | Снят: тот же `/api/contractors` с теми же фильтрами, что каталог на главной. `/contractors` — постоянный редирект. Каталог сокращён с восьми колонок до пяти |

Проверки агента: `tsc` в обоих пакетах, backend unit 646/646, frontend 37/37, `build`, `check:labels` — зелёные.
**Не проверено:** живой прогон с LM Studio и рабочей базой, e2e Playwright, поведение автоповтора на реальных падениях.
...
### Задача владельца 21.09.2026 — портал просматривает, а не публикует

Пять правок по замечаниям владельца, одним заходом (не этап пакета):

1. **Обработка без оператора.** Из интерфейса убраны «Поставить на разбор», «Повторить», «Отменить»,
   «Опубликовать этот набор»; `/admin/process` и карточка запуска — только чтение, панель набора —
   `CandidateSetPanel`. Серверные маршруты, `publish.ts` и CLI не тронуты. Слово «публикация» как действие
   с экранов убрано.
2. **Строки кликаются целиком** — каталог компаний и лента «Последнее» (`.row-link` в `src/index.css`,
   проверка перехода по пустой ячейке — в `e2e/dailyRoute.spec.ts`).
3. **Карточка компании: `signals@2`** — договоры, корпоративные связи, контрагенты, пакеты работ, события
   по видам, число судебных дел, края выборки по датам. Панель показателей вынесена во всю ширину, плитки
   больше не рвут подписи. Старые снимки остаются `signals@1`: числа появятся после `metrics:refresh`.
4. **Тема публикации моделью (headline@1)** — миграция `027_revision_headlines.sql`, `llm/headline/*`,
   `headline/service.ts`, `npm run pipeline:once -- --headlines`, флаги `HEADLINE_*`. Тема помечена как
   машинная, доказательством не является.
5. **Страница документа** отвечает «что портал взял из этого текста»: `GET /api/items/:id/extraction`,
   состояние словами, утверждения с цитатами и ссылками на карточки, текст и версии — ниже.

Следом, по тем же скринам: схема связей больше не печатает пояснения дважды (`graph.notes` дублировались
внизу панели) и не занимает пол-карточки — фильтры свёрнуты там, где схема боковая панель; строки
«Подрядчиков» и мобильные карточки кликаются целиком, как в каталоге. Новый эндпоинт
`/api/items/:id/extraction` покрыт интеграционно в `reprocess.int.test.ts` (состояние, цитаты дословно
из редакции, 404 на неизвестную публикацию).

Офлайн-проверки: backend typecheck 0, `npm test` 640+4 новых файла; frontend typecheck 0, `npm test` 34,
`check:labels` ok, `build` + `check:build` ok. Полный прогон vitest на фронте на этой машине требует
`--maxWorkers=1`: при свободных ~1 ГБ ОЗУ параллельные воркеры падают по памяти (к коду отношения не имеет).

**За пользователем:** `npm run migrate` (027), `npm run test:integration` (в т.ч. новый
`headline/headline.int.test.ts`), `npm run metrics:refresh` (иначе новых чисел не будет),
`npm run pipeline:once -- --headlines --limit 50` для уже собранных текстов, `release:check` /
`release:manifest`, `npm run e2e`.

### Этап 20C — только перечень оператора и импорт файла (2026-09-21)

Прогон O3 дал **BLOCKED, HTTP 403**: наш.дом.рф отвечает браузеру и не отвечает порталу, ключ API
исключён владельцем. По требованию владельца обход каталога удалён из схемы профиля (не выключен
флагом), сбор идёт только по перечню оператора; добавлен импорт сохранённого ответа из файла
(`--registry-import`, `--registry-objects`). Миграций нет. Офлайн: tsc 0, 644/644.
Отчёт — `stages/20C_REPORT.md`, прогон пользователя — `TESTING_LOCAL.md` раздел **Q**.
