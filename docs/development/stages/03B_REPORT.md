...
Статус: **PASS** (2026-09-14, прогон 2 пользователя @ `a00a969` — [evidence/03B/USER_RUN.md](../evidence/03B/USER_RUN.md)): unit 22 / 325, интеграция 10 файлов / 103.
Прогон 1 (`7e1cf2d`): unit 22 / 325, раздел F (CLI) — ок; интеграция 101 / 103 — два падения, оба исправлены (см. «Прогон 1»).
...
| Полный путь, evidence → chunk → run → revision, проекция карточки, `reviewed_supported` машиной не ставится | `reprocess.int.test.ts` | tg_info_test | 0 | PASS | прогон 2, evidence/03B/USER_RUN.md |
| Падение последнего чанка → partial; повтор новым запуском | то же | tg_info_test | 0 | PASS | прогон 2 |
| Непокрытый хвост → failed без вызова модели | то же | tg_info_test | 0 | PASS | прогон 2 |
| Timeout → исход `timeout`, failed | то же | tg_info_test | 0 | PASS | прогон 2 |
| Crash до commit итога → тот же запуск продолжается без повторных вызовов | то же | tg_info_test | 0 | PASS | прогон 2 |
| Crash после commit → повторного захвата нет, набор один | то же | tg_info_test | 0 | PASS | прогон 2 |
| Crash внутри публикации → контрольные количества не изменились | то же | tg_info_test | 0 | PASS | прогон 2 |
| Два worker'а: один захват; прежний держатель после перехвата не пишет | то же | tg_info_test | 0 | PASS | прогон 2 |
| Поздний старый разбор → `rejected_stale`, указатель не тронут, история | то же | tg_info_test | 0 | PASS | прогон 2 |
| Нерелевантная новая версия при двух источниках и ручном решении: решение и чужое доказательство на месте, `needs_revalidation`, компании не удалены, строки evidence не удалены | то же | tg_info_test | 0 | PASS | прогон 2 |
| Набор старой редакции после появления новой → устаревший | то же | tg_info_test | 0 | PASS | прогон 2 |
| Отзыв права ИИ до публикации → `rejected_policy`; захват и постановка запрещены | то же | tg_info_test | 0 | PASS | прогон 2 |
| Одинаковые имена с разными ИНН → две компании | то же | tg_info_test | 0 | PASS | прогон 2 |
| Идемпотентный повтор публикации; 409 на версию (функция и API); без CSRF — 401/403 | то же | tg_info_test | 0 | PASS | прогон 2 |
| Legacy-событие замещённого документа скрыто, подтверждённое вручную — видно, строки не удалены | то же | tg_info_test | 0 | PASS | прогон 2 |
| Повторная публикация новой редакции не размножает объект без города и компанию без ИНН | то же | tg_info_test | 0 | PASS | прогон 2 |
| CLI: `--runs`, `--preview`, `--publish` (stale, published, already), `--reextract` без лимита, `--merge` заблокирован | TESTING_LOCAL F | tg_info_test | 0 | PASS (прогон 1) | evidence/03B/USER_RUN.md |
| Регрессия 01–03A (9 файлов / 82) | `npm run test:integration` | tg_info_test | 0 | PASS (прогон 1) | 9 файлов зелёные |
...
## Прогон 1 и исправления

| Тест | Причина | Исправление |
|---|---|---|
| публикация → `card_events_v` 1 вместо 2 | дефект кода: два суда без даты, суммы и контрагента давали один `content_key`, публикация сливала их в одно утверждение | колонка `assertions.event_discriminator` в 013 (хэш нормализованной цитаты недоопределённого события), входит в ключ и в триггер неизменяемости; подпись предпросмотра учитывает его; ключи прежних строк не меняются |
| API: POST без CSRF → 200 | дефект теста: клиент сам подставляет cookie и CSRF | явные `{ cookie: '' }` → 401 и `{ 'x-csrf-token': '' }` → 403, версия публикации не изменилась |

Остальные сценарии интеграции и весь раздел F прошли в прогоне 1.
...
- Недоопределённое событие (без даты, суммы и контрагента) из двух источников с разной формулировкой — два утверждения,
  а не одно с двумя основаниями: консервативно, объединение — решением человека или этапом 06.
