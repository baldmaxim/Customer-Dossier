...
Статус: **код готов, ждёт прогона пользователя** (TESTING_LOCAL A5 — 12 файлов / 135 тестов, раздел H).
...
| TC-042 RSS-анонс → полная статья с отрицанием; недоступная статья → честный excerpt | `sites.int.test.ts` | tg_info_test | — | NOT_RUN | ждёт прогона пользователя |
| Повтор без дублей; 304 по ETag — без новой редакции; правка статьи при refetchKnown — новая редакция | то же | tg_info_test | — | NOT_RUN | — |
| Редирект статьи вне allowlist — запрос не уходит, анонс с причиной `policy` | то же | tg_info_test | — | NOT_RUN | — |
| TC-043 HTML-список с пагинацией, даты зоны профиля, query-sensitive canonical | то же | tg_info_test | — | NOT_RUN | — |
| TC-044 сбой второй страницы: курсор не ушёл дальше, повтор дочитывает; лимит страниц и хвост | то же | tg_info_test | — | NOT_RUN | — |
| TC-046 отсутствие селектора / смена вёрстки — parser_degraded, курсор не двигается | то же | tg_info_test | — | NOT_RUN | — |
| TC-045 429 с Retry-After без роста fail_streak; 403 без обхода; oversize; неверный профиль без запросов | то же | tg_info_test | — | NOT_RUN | — |
| Карточка объекта: повтор — `unchanged` с версией парсера, не новая публикация; проба без записи | то же | tg_info_test | — | NOT_RUN | — |
| Регрессия 01–04 (11 файлов / 123) | `npm run test:integration` | tg_info_test | — | NOT_RUN | ожидается 12 / 135 |
| Seed, база, CLI, админка | TESTING_LOCAL H | tg_info_test | — | NOT_RUN | — |
