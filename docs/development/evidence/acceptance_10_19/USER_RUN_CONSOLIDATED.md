...
|---|---|---|---|---|
| C1 | `evidence/18/USER_RUN.md` B1–B4; сценарии `evidence/12` (T12-12), `15A` шаг 4, `15B` шаг 4, `17` шаг 4 | на `tg_info_test` после B3; E2E создаёт снимки. **E2E — на `npm run preview` (127.0.0.1:4173)**: под `npm run dev` service worker выключен и кэш-проверка T18-02 пуста. Адрес фронтенда должен быть в `CORS_ORIGINS` API: в окне API `$env:CORS_ORIGINS='http://127.0.0.1:4173'` (переменная окна важнее `.env`, файл не меняется), `$env:E2E_BASE_URL='http://127.0.0.1:4173'`; иначе вход — `bad_origin` | снимки, решения | 6 тестов × 2 размера passed; ручные задачи — PASS/FAIL с затруднениями; PDF и 390 px — решение человека |
