...
Статус: **VERIFIED по отчёту пользователя A3 (2026-09-17)** — sharp (ограничение принято владельцем); backend `qs`/`express` — **ACC-04 исправлен вариантом 1 (код, 2026-09-18), зависимость остаётся в lock**.
...
## Backend: `qs` и `express` (ACC-04 — путь устранён кодом)
| Поле | Значение |
|---|---|
...
**Решение владельца (2026-09-18): вариант 1.** `backend/src/app.ts`: `app.set('query parser', 'simple')` и `rejectStructuredQuery`
на `/api` (после Host/Origin, до авторизации): ключ со скобками или повторённый ключ — 400 `invalid_query`. Тесты —
`backend/src/api/queryParser.test.ts` (вложенный параметр, повторённый ключ → 400; плоские параметры проходят до авторизации → 401).
Зависимости не менялись: `qs@6.15.3` остаётся установленным (через `express`), но строку запроса API больше не разбирает;
`express.urlencoded` не подключён. Остаточный риск: уязвимая версия в lock до планового обновления express (вариант 2, отдельной задачей).
Проверка: повтор A0 (новый отпечаток) и A1, затем B1 на исправленном коде.
