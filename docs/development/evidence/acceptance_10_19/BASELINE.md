...
| Изменение кода после `d33521b` | **ACC-04** (2026-09-18): `backend/src/app.ts`, `backend/src/api/queryParser.test.ts`. **ACC-05** (2026-09-18): `backend/src/reprocess/provider.ts`, `executionIdentity.test.ts`. Зависимости не менялись. Каждый код привязывается своим отпечатком: A0/A1 — `d33521b`, A0r/A1r и B1 (FAIL) — `cd4b7e8`, после ACC-05 — новый повтор |
