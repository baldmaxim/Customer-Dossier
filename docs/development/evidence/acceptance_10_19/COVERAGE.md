...
| AC-10 | 10,19 | API на копии + `release:probe --compare`, контролируемая запись после сравнения | W | D,B | B4 | B4 (18.09, `tg_info_test_restore_1019`): **PASS** — откат цитаты E7 (`UPDATE 1`) → manifest MATCH с `B3-before-manifest.json`; API на копии, фон выключен; `release:probe --compare` с `B3-probe-before.json` — совпадает (snapshot, export html/md/json, case_dossier — HTTP 200, `integrity.verified=true`); контролируемая запись: обращение 4 (`replayed=False`) и снимок 4 без `duplicate key` — последовательности восстановлены. После записи compare не выполнялся |
...
| AC-35 | 18 | `dossierViews.test` logout/недоверенная строка; E2E T18-02/03; `check:build` с canary | O-часть **R**, B — W | O,B | A2, C1 | O-часть **PASS** (A2: компонентные тесты, canary в бандле не найден); браузер — NOT_RUN. Под `npm run dev` service worker выключен (`devOptions.enabled=false`) — кэш-часть T18-02 там пуста по построению; C1 выполняется на `npm run preview` (сборка с SW) |
