...
**Сейчас проверяется этап 08A** (рабочее досье: обращение, объект, проверка): шаги A1–A5, затем раздел L.
Разделы B–K — проверки этапов 01–07 (пройдены), повторять не обязательно.
...
Ожидается: typecheck без ошибок; unit — **28 файлов / 410 тестов passed**; сборка frontend успешна.
...
Ожидается: `[integration] тестовая цель: 127.0.0.1:55433/tg_info_test`, затем **16 файлов / 173 теста passed**:
...
| `db/migrate.int.test.ts` | dry-run без DDL, отказ destructive без флага, миграции 001–019 |
