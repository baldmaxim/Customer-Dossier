...
**Сейчас проверяется этап 09** (сквозная приёмка, копия и восстановление, замеры): шаги A1–A5, затем раздел N.
Разделы B–M — проверки этапов 01–08B (пройдены), повторять не обязательно.
...
2. Остановите API (Ctrl+C) и обновите обращение: вместо досье — «Обращение недоступно: …», прежние данные не показаны как свежие.
   Запустите API.
3. Во второй вкладке нажмите «Выйти», в первой перейдите на другое обращение: экран входа, данные прежней сессии не видны.
...
4. Во второй вкладке «Выйти», в первой обновите снимок: экран входа. Прямая ссылка `/api/snapshots/1/export.html` в новой
   вкладке без входа — `401`.
...
«Дельта-Релиз»; строка вида `публикаций 11, редакций 12 (правка новости: revision_created)`; объект
...
команда напечатает таблицу и разницу и завершится с ненулевым кодом.

```powershell
docker exec tg-info-test-db psql -U tg_test -d tg_info_test_restore -c "DELETE FROM review_decisions WHERE id = (SELECT max(id) FROM review_decisions);"
```
...
4. **Обращение**: создать по компании и объекту, корпус 2, вид работ «монтаж систем ВК», роль «подрядчик».
   В досье: роль — источники противоречат; два судебных дела разными строками (истец и ответчик);
...
5. **Выход**: во второй вкладке «Выйти», в первой обновить досье — экран входа.
...
Ожидается **18 файлов / 194 теста passed**, включая `release/release.int.test.ts`.
