...
регулярной проверки установки достаточно A1–A5 и N2 (`release:check`).
...
| Файл | Что проверяет |
|---|---|
| `release/release.int.test.ts` | **этап 09**: TC-074…TC-077 — установка с нуля, повтор и dry-run без записи, апгрейд базы, накаченной до 009, без потери legacy-данных и без выдуманного допуска; путь источник → публикация → редакция → запуск → утверждения с доказательствами → одноимённые юрлица → противоречие → решение аналитика → сигналы → обращение → досье → снимок → выгрузка; перезапуск приложения не меняет снимок; правка, переразбор, слияние и переименование не трогают старый снимок, новый их видит; модель недоступна и ответ не по схеме — канон не меняется; повтор публикации не удваивает события; отозванный допуск отказывает с причиной; запрос с чужого Origin и Host — отказ; опасный фрагмент не исполняется в выгрузке; контрольные числа и их расхождение |
| `snapshot/snapshot.int.test.ts` | **этап 08B**: TC-069…TC-073 — цепочка договоров без транзитивного ребра, ребро участия с корпусом и работами открывает своё основание; снимок S1 с цитатами и редакциями, повтор по ключу; новая публикация (отрицание), переименование, слияние, решение аналитика и пересчёт не меняют S1, S2 их учитывает; UPDATE/DELETE снимка запрещены; срез знаний на прошлую дату — 422, фильтр дат исключает событие 2024; HTML/MD/JSON без сети, опасная цитата не исполняется; отзыв допуска скрывает цитаты при выдаче; вымарывание с журналом hash |
| `dossier/dossier.int.test.ts` | **этап 08A**: TC-065…TC-068 — два одноимённых юрлица в поиске, обращение по выбранному без изменения канона и с идемпотентностью, досье с ролью и цитатой, юрлицо не установлено, противоречие в досье и очереди, причина решения обязательна, решение при повторном открытии, 409 второй вкладки и история, пагинация, досье без сети, перенос обращения при слиянии |
...
| `db/migrate.int.test.ts` | dry-run без DDL, отказ destructive без флага, миграции 001–020 |
...
2. Одобрять источники не нужно. Запустить API и UI (два окна; в окне API задать переменные `DATABASE_URL`,
   ...
   ```
3. Открыть адрес из шага 1 — вход не требуется.
4. Ожидается:
...
Портал открывается без входа; «Админка» → «Источники»: «сбор: …, ИИ: …» и причина; «Допуск»:
разрешение без основания не сохраняется; «Слить (выключено)»; DevTools → Application →
...
## L. Этап 08A — рабочее досье: обращение, объект, проверка

LM Studio не нужен и для L3 должен быть **выключен**.

### L1. Данные

Схема с нуля (как в H1), затем seed без `DATABASE_URL`:
...
Remove-Item Env:DATABASE_URL
$env:TEST_DATABASE_URL = 'postgresql://tg_test:tg_test@127.0.0.1:55433/tg_info_test'
npm run seed:test-dossier
$env:DATABASE_URL = 'postgresql://tg_test:tg_test@127.0.0.1:55433/tg_info_test'
```

Ожидается: две строки `компания #… Альфа-Демо: inn 5001007329` и `… Альфа-Демо: inn 7700000016`, `Бета-Демо`, `Гамма-Демо`
(`реквизитов нет`), `объект «Берег-Демо» #…`, `сигналы: succeeded`.

### L2. Сценарий в браузере

API (`npm run dev` с переменными раздела B) и UI (`frontend: npm run dev`), вход токеном. В меню появились «Обращения» и «Проверка».

1. **Поиск** «Альфа-Демо»: две строки с разными ИНН и «одноимённых: 1 — сверьте реквизиты». Откройте компанию с ИНН 5001007329.
2. **Карточка компании**, блок «Резюме»: «Идентификация: реквизит с верной контрольной суммой», «участии в 1 объектах: подрядчик — 1»,
   события (дел, где компания истец… — 1). «Контрагенты»: совместное участие «Бета-Демо (генподрядчик) на Берег-Демо» — с пометкой
   «не договор». «Противоречия»: «противоречие источников · утверждение #…».
3. «Создать обращение» → форма с выбранной компанией. Объект: поиск «Берег» → «Берег-Демо · комплекс». Корпус «корпус 2», вид работ
   «монтаж систем ВК», роль «подрядчик», заказчик со слов — выбрать «Бета-Демо», условия «аванс 30 % со слов». «Сохранить обращение».
4. **Обращение**: «Главное по источникам» — первым «В публикации сообщается: Альфа-Демо — не подрядчик на объекте Берег-Демо» с цитатой;
   «Роль: источники противоречат»; «Со слов обратившегося компания — подрядчик… Это запись оператора»; «Кто заказывает работы:
   договорная цепочка не установлена», совместное участие Бета-Демо; «Условия» — только со слов; контекст — «задержка на объекте
   (2024 год); относится к корпус 1; событие вне периода участия компании»; судебное дело «компания — истец… не означает нарушения».
   «Вопросы контрагенту»: о противоречии роли, «Кто заказчик работ и на основании какого договора», о судебных делах. Про аванс вопроса нет
   (условия записаны со слов).
5. «основания (утверждение #…)» у любой фразы — карточка утверждения с доказательствами, цитатой и историей решений.
6. **Проверка** → «противоречие источников» → «сравнить версии»: слева утверждение об участии, справа отрицание. Слева выберите
   решение «спорно» — кнопка неактивна, пока нет причины; укажите причину → «Решение записано».
7. Вернитесь в обращение (перезагрузка): у роли «спорно по решению аналитика»; в карточке утверждения — решение с причиной.
8. **Две вкладки**: откройте обращение в двух вкладках, в обеих «Изменить обращение». Сохраните в первой, затем во второй —
   «Обращение изменено в другой вкладке…», данные второй вкладки не записаны. Внизу «История обращения» — версии 1 и 2.
9. «Обращения» → «Новое обращение» → «юрлицо не установлено», название «Альфа-Демо» → сохранить. В досье: «Юрлицо не установлено»,
   «есть компании с таким же названием (2): … — сведения о них в досье не смешиваются», роль «юрлицо не установлено», вопрос о ИНН/ОГРН.
10. **Объект** (ссылка из обращения или карточки): таблица участников — Альфа-Демо (подрядчик, корпус 2, ВК, с 01.06.2026) и Бета-Демо
    (генподрядчик, период не указан); «Не учтено как участие» — отрицание. Период 01.01.2024–31.12.2024: Альфа-Демо «вне выбранного
    периода», Бета-Демо «период участия не указан»; галочка «только пересекающиеся» скрывает обоих — пересечение ни у кого не подтверждено.
11. «Админка» → «Источники»: у `demo_dossier_channel` строка «успешных запусков не было · публикаций 5, с неполным текстом 5» (у синтетических документов полнота не указана и честно считается неполной).

### L3. Без модели, ошибки и сессия

1. LM Studio выключен: карточка, обращение, объект и проверка открываются, у обращения видны «Собрано …» и срез сигналов.
2. Остановите API (Ctrl+C) и обновите обращение: вместо досье — «Обращение недоступно: …», прежние данные не показаны как свежие.
   Запустите API.
3. DevTools → Application → Cache Storage: ответов `/api` нет (данные досье офлайн не читаются).
4. Ширина 390 px: обращение, форма и объект без горизонтального скролла страницы (таблица участников скроллится внутри).
5. По желанию: скриншоты шагов L2.4, L2.6 и L2.10 положите в `docs/development/evidence/08A/`.

### L4. Откат интерфейса (по желанию)

```powershell
cd frontend
$env:VITE_DOSSIER_UI = 'false'; npm run build; Remove-Item Env:VITE_DOSSIER_UI
npx vite preview
```

В меню нет «Обращения» и «Проверка», адрес `/cases` ведёт на поиск; прежние экраны работают. Пересоберите без флага.

## M. Этап 08B — схема связей, снимки досье, выгрузки

LM Studio не нужен. Данные — те же, что в L1 (`seed:test-dossier`); если база уже заполнена после L — повторять не нужно.

### M1. Данные и сервер

Схема с нуля и seed — как в L1. API (`npm run dev` с переменными раздела B) и UI (`frontend: npm run dev`), вход токеном.
`GRAPH_EXPORT_ENABLED` не задавайте (по умолчанию включено).

### M2. Схема связей

1. Карточка компании «Альфа-Демо» (ИНН 5001007329) → блок «Схема связей» → «Показать схему». Узел компании с рамкой-акцентом,
   объект «Берег-Демо», на глубине 2 — «Бета-Демо». Под схемой: «Рёбра — только утверждения со своим основанием…».
2. В легенде фильтров типы различаются штрихом линии и подписью; «совместное упоминание» по умолчанию выключено.
   Толщина и цвет рёбер одинаковые.
3. Нажмите на ребро (или Tab до ребра и Enter) участия «Альфа-Демо → Берег-Демо»: ниже подпись «участие в объекте · подрядчик ·
   корпус 2 · ВК», период и число оснований, карточка утверждения с цитатой, редакцией и историей решений.
4. «Таблица»: те же связи строками — от, связь, к, период, статус, «утверждение #…». Отрицание участия в схеме не видно; галочка
   «показать планы, заявления и отрицания» его добавляет.
5. Глубина 1: «Бета-Демо» пропадает, в примечаниях «Глубина ограничена 1…». Период 01.01.2024–31.12.2024: ребро участия
   Альфа-Демо (с 01.06.2026) исчезает.
6. Страница объекта «Берег-Демо» и обращение (из L2.3) — там такой же блок схемы.

### M3. Снимок S1, изменения, снимок S2

1. Откройте обращение из L2.3 → «Снимки досье» → «Создать снимок» (без дат). Появилась строка «Снимок №1 · … · версия обращения …»
   и hash. Двойной клик по кнопке второй снимок не создаёт.
2. Откройте снимок: «Целостность подтверждена · sha256-canonical-json@1 …», версии шаблона, правил и схемы; разделы досье,
   схема «(из снимка)», решения аналитика, таблица источников с номером редакции, ограничения. Запомните первые символы hash.
3. Измените состояние портала:
   - вкладка «Проверка» → любое утверждение → решение «спорно» с причиной;
   - «Изменить обращение» → поменяйте примечание и сохраните;
   - по желанию — пересчёт сигналов (`npm run metrics:refresh`).
4. Снова откройте снимок №1: hash и содержание прежние (новое решение и новое примечание в нём не видны).
5. Создайте снимок №2: hash другой, в «Решениях аналитика» — новое решение с причиной, версия обращения больше.
6. Снимок №3 с датами «События и роли с 01.01.2026 по 31.12.2026»: в шапке «Фильтр дат…», в ограничениях — «Это не документ,
   существовавший в прошлом»; судебного дела 2024 года в «Событиях» нет.
7. Срез знаний на прошлую дату в интерфейсе не предлагается; отказ API (`422 historical_cutoff_unsupported`) проверяет интеграционный тест.

### M4. Выгрузки

1. В снимке №1 «Версия для печати» — новая вкладка: заголовок, компания и ИНН, наблюдения, роль, события, вопросы, таблица связей,
   источники с цитатами и редакциями, ограничения, hash. В DevTools → Network — один запрос, внешних ресурсов нет.
   Ctrl+P → «Сохранить как PDF»: таблицы не обрезаны справа, русский текст читается.
2. «Скачать Markdown» и «Скачать JSON» — файлы `dossier-snapshot-1.md` / `.json`; в JSON `"exportSchema": "dossier-snapshot-export@1"`.
   Название обращения, компания и hash совпадают во всех трёх выгрузках.
3. В выгрузках нет токена, cookie, stack trace и адресов LM Studio.

### M5. Отзыв допуска, выход, вымарывание

1. «Админка» → «Источники» → `demo_dossier_channel` → допуск на сбор «отозван» с основанием → сохранить.
2. Откройте снимок №1: предупреждение «Сейчас скрыто цитат: N — допуск источников изменился…», в источниках вместо цитат
   «Скрыто при выдаче: …», «Целостность подтверждена» осталось. «Версия для печати» — цитат этого источника нет.
   Верните допуск как был.
3. В снимке №1 «Вымарывание фрагмента»: выберите фрагмент, основание «проверка вымарывания» → «Вымарать». Цитата заменена
   «[фрагмент вымаран по решению оператора]», в журнале строка с основанием, целостность подтверждена (hash новый).
4. Прямая ссылка `/api/snapshots/1/export.html` в новой вкладке открывается: входа нет, портал доступен локально.
   Тот же адрес с чужим `Origin` (DevTools → Network → Edit and Resend) — `403`.
5. Ширина 390 px: снимок и схема без горизонтального скролла страницы (схема и таблицы скроллятся внутри).

### M6. Откат (по желанию)

Остановите API, запустите с `$env:GRAPH_EXPORT_ENABLED = 'false'` (bash: `GRAPH_EXPORT_ENABLED=false npm run dev`):
схема — «Схема недоступна: Схема и выгрузки отключены…», «Создать снимок» — та же ошибка; существующий снимок открывается,
...
### N1. Установка с нуля и данные

Схема с нуля, синтетический корпус приёмки:
...
Ожидается: компании «Порт-Релиз», «Бета-Релиз», «Альфа-Релиз» (два юрлица с разными ИНН), «Гамма-Релиз»,
«Дельта-Релиз»; строка вида `публикаций 13, редакций 14 (правка новости: new_revision)`; объект
«Причал-Релиз»; `сигналы: succeeded`.

### N2. Контрольные числа и связность
...
Ожидается: `local-inventory@1`, список непустых таблиц, `связность: нарушений нет`, источники
`demo_release_*` с допуском `approved` и пометкой «синтетический», флаги `INGEST_ENABLED=false`,
`PIPELINE_ENABLED=false`. Команда ничего не пишет в базу.

### N3. Резервная копия и восстановление в отдельную цель (TC-075)

```powershell
docker exec tg-info-test-db pg_dump -U tg_test -Fc -d tg_info_test > backup-release.dump
docker exec tg-info-test-db psql -U tg_test -d postgres -c "CREATE DATABASE tg_info_test_restore;"
docker exec -i tg-info-test-db pg_restore -U tg_test -d tg_info_test_restore --no-owner < backup-release.dump
...
Ожидается: `совпадает с before.json` и строка о том, что сравниваются разные базы.
Дамп не класть в Git и не присылать в переписку — он с данными.

Проверка, что расхождение видно: в восстановленной базе удалите одну строку и повторите `--compare` —
команда напечатает таблицу и разницу и завершится с ненулевым кодом. (Решений аналитика в seed нет —
удаляйте строку из `merge_queue`.)

```powershell
docker exec tg-info-test-db psql -U tg_test -d tg_info_test_restore -c "DELETE FROM merge_queue WHERE id = (SELECT max(id) FROM merge_queue);"
```

### N4. Сквозной сценарий в браузере (TC-076)

API (`npm run dev`) и UI (`frontend: npm run dev`), вход токеном.

1. **Поиск** «Альфа-Релиз» — две строки с разными ИНН и пометкой об одноимённых. Открыть юрлицо с ИНН,
   у которого есть участие на корпусе 2.
2. **Карточка**: резюме, сигналы с правилами, контрагенты (генподряд и субподряд — отдельно от совместного
   участия), противоречия. Схема связей: цепочка «Порт-Релиз → Бета-Релиз → Альфа-Релиз», прямого ребра
   «Порт-Релиз → Альфа-Релиз» нет.
3. **Объект** «Причал-Релиз»: корпус 1 — смена подрядчика («Гамма-Релиз» до марта 2026, «Дельта-Релиз» с апреля),
   корпус 2 — участие и отрицание рядом.
4. **Обращение**: создать по компании и объекту, корпус 2, вид работ «монтаж систем ВК», роль «подрядчик».
   В досье: роль — источники противоречат (и после решения аналитика: отрицание не исчезает, решение видно в атрибуции); два судебных дела разными строками (истец и ответчик);
   перепечатки одной новости показаны как публикации, а не как три подтверждения.
5. **Проверка**: разобрать противоречие — решение с причиной. Вернуться в обращение: атрибуция изменилась.
6. **Снимок**: создать, открыть, «Версия для печати» → Ctrl+P → PDF; скачать Markdown и JSON.
7. **Перезапуск**: остановить API (Ctrl+C), запустить снова, открыть тот же снимок — hash и содержание прежние.

### N5. Отказы (TC-077)

1. **БД недоступна**: остановите контейнер (`docker stop tg-info-test-db`) и обновите карточку — видно
   сообщение об ошибке, а не «сведений нет». Запустите контейнер снова (`docker start tg-info-test-db`).
2. **Модель недоступна**: LM Studio выключен — карточки, досье и снимки работают на накопленных данных;
   `npm run pipeline:once -- --check` честно сообщает, что модель недоступна.
3. **Сеть**: отключать не требуется — портал не ходит в интернет при чтении досье; в DevTools → Network
   на странице снимка нет внешних запросов.
4. **Допуск отозван**: в админке снимите ИИ-обработку у `demo_release_channel`, затем
   `npm run pipeline:once -- --reextract --limit 1 --source demo_release_channel` — запуск не ставится,
   причина названа. Верните допуск.
5. **Чужая страница**: запрос к `/api` с посторонним `Origin` — `403` (вход снят, этот гард остался).

### N6. Замеры (TC-078)

```powershell
cd backend
npm run release:bench -- --runs 5 --out bench-release.json
```

Ожидается: строка с машиной (ОС, процессор, ядра, память, Node), объём данных, затем медианы по шагам:
поиск, карточка, резюме, схема связей, досье объекта и обращения, создание снимка, открытие снимка,
выгрузка HTML. Пришлите этот вывод — числа идут в отчёт как факт, а не как обещание.

### N7. Интеграция целиком
...
Ожидается **18 файлов / 196 тестов passed**, включая `release/release.int.test.ts`.

### N8. Уборка тестовых баз

```powershell
docker exec tg-info-test-db psql -U tg_test -d postgres -c "DROP DATABASE tg_info_test_restore;"
Remove-Item backup-release.dump, before.json, bench-release.json -ErrorAction SilentlyContinue
```
...
- `blocked` (403) — источник не отдаёт данные этой машине. Не обходить.
  **Прогон 21.09.2026: именно этот исход.** наш.дом.рф открывается браузером и отвечает 403 порталу,
  ключ API исключён владельцем. Рабочий путь — импорт файла, раздел Q.

## P. Этап 20B — карточки из реестра

### P1. Миграция и тесты

```powershell
cd backend
npm run migrate -- --dry        # в плане 026_registry_canon.sql, destructive — нет
npm run migrate
$env:TEST_DATABASE_URL = 'postgresql://tg_test:tg_test@127.0.0.1:55433/tg_info_test'
npm run test:integration -- src/ingest/registry/registry.int.test.ts
cd ../frontend; npm test; npm run check:labels; npm run build
```

Ожидается: в канон записаны юрлицо застройщика с ИНН (`entity_identifiers.origin = registry`),
объект, роль `developer` со статусом `text_grounded` и связь с группой компаний; цитата
доказательства — целая строка редакции; повтор сбора утверждений не добавляет; срок сдачи
утверждением не становится.

### P2. Глазами, на тестовой базе

Открыть карточку объекта: блок «Данные реестра» с датой сведений, атрибуцией, полями и разделом
«Что изменилось в реестре» после второго сбора с изменённым сроком. Проверить на 390 px: строки
переносятся, горизонтальной прокрутки нет. Слова «проверено» в блоке быть не должно.

### P3. Выключатель

`REGISTRY_PUBLISH_ENABLED=false` в `.env` (правит только пользователь) → сбор идёт, снимки
пишутся, новых утверждений нет. Уже записанное остаётся.

## Q. Этап 20C — импорт записи реестра из файла

Сети не требуется. Нужен зарегистрированный источник-реестр с профилем (раздел O) и **допуском на сбор**:
импорт — это сбор, допуск проверяется так же.

### Q1. Сохранить ответ

В браузере откройте запись объекта, DevTools → Network → нужный ответ → «Copy response» либо
«Save as». Сохраните как `backend/answer.json`.

### Q2. Импорт

```powershell
cd backend
npm run ingest:once -- --registry-import <ключ> --file answer.json
npm run ingest:once -- --registry-objects <ключ>
```

Ожидается при совпавшей карте полей:
`[registry] object 62087 «…»: новая запись, полей N`, затем `снимок записан, утверждений 1–2`.
Вторая команда печатает строку с числом снимков и датой сведений.

Если карта полей не совпала: `карта полей не совпала…` и строка `пути в ответе: …`. Перепишите
`identity`/`fields` в профиле по этим путям и повторите — сеть при этом не задействована.

### Q3. Что проверить дальше

| Действие | Ожидается |
|---|---|
| повторить импорт того же файла | `без изменений`, второй редакции и второго снимка нет |
| сохранить ответ повторно через время и импортировать | при изменившемся поле — `данные изменились, сохранена новая редакция` |
| открыть карточку объекта | блок «Данные реестра» с датой сведений и, после второго импорта, «Что изменилось в реестре» |
| `npm run ingest:once -- --source <ключ>` | при пустом `objectIds` записей не собирает: каталог не обходится |

## R. Этап 22: поток разбора и карточка компании

Проверяется то, что агент не мог проверить сам: новые SQL против настоящего PostgreSQL и
поведение конвейера, когда LM Studio выключен.

### R1. Интеграционный тест новых запросов
...
Ожидается: зелёные, в том числе `src/api/companyCard.int.test.ts` (лента публикаций,
контрагенты, состояние обработки). Красный здесь — опечатка в имени колонки или представления.

### R2. Разбор при выключенной модели

1. Закройте LM Studio (или остановите сервер в нём).
2. `cd backend && npm run dev`.

| Ожидается | Почему это важно |
|---|---|
| в логе `[pipeline] проход пропущен — модель не отвечает…`, один раз, а не каждые 30 с | запуски не ставятся и не сгорают |
| в базе не появляется новых строк `extraction_runs` со статусом `failed` | раньше редакция выпадала из автопотока навсегда |
| после запуска LM Studio в логе `[pipeline] модель снова отвечает` и разбор идёт | поток восстанавливается сам, без команд |

### R3. Возврат упавшего разбора

Если в базе есть запуски `failed`/`partial` (например, после R2 на старых данных):

```sql
SELECT id, revision_id, status, finished_at, previous_run_id FROM extraction_runs ORDER BY id DESC LIMIT 20;
```

Через `REPROCESS_RETRY_BACKOFF_MIN` (по умолчанию 15 минут) при поднятой модели у такой редакции
должен появиться новый запуск с `previous_run_id` прежнего. Прежний запуск не меняется.
После `REPROCESS_RETRY_MAX` (3) неудач на редакцию повторы прекращаются — на `/admin` это
состояние «разбор упал, попытки исчерпаны».

### R4. Экраны

| Где | Ожидается |
|---|---|
| `/admin` | ступень «Обработка» показывает состояния публикаций словами и ярлык «Модель: отвечает / не отвечает» |
| карточка компании, вкладка «Обзор» | сводка, лента публикаций с цитатами, блок «С кем работает» |
| карточка компании, вкладка «Подробно» | прежние сигналы, досье, объекты, события, схема — числа те же |
| `/contractors` | редирект на главную; в меню пункта «Подрядчики» нет |
